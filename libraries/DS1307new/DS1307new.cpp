...
void DS1307new::setTime(void)
{
  Wire.beginTransmission(DS1307_ID);
  printIIC(0x00);
  printIIC(dec2bcd(second) | 0x80);   // set seconds (clock is stopped!)
  printIIC(dec2bcd(minute));           // set minutes
  printIIC(dec2bcd(hour) & 0x3f);      // set hours (24h clock!)
  printIIC(dec2bcd(dow+1));              // set dow (Day Of Week), do conversion from internal to RTC format
  printIIC(dec2bcd(day));             // set day
  printIIC(dec2bcd(month));            // set month
  printIIC(dec2bcd(year-2000));             // set year
  Wire.endTransmission();
}
...
{
  Wire.beginTransmission(DS1307_ID);
  printIIC(0x07);                      // set CTRL Register Address
  Wire.endTransmission();
  ...
  ctrl = readIIC();                // ... and store it in ctrl
}
...
void DS1307new::setCTRL(void)
{
  Wire.beginTransmission(DS1307_ID);
  printIIC(0x07);                      // set CTRL Register Address
  printIIC(ctrl);                      // set CTRL Register
  Wire.endTransmission();
}
...
void DS1307new::getRAM(uint8_t rtc_addr, uint8_t * rtc_ram, uint8_t rtc_quantity)
{
  ...
  printIIC(rtc_addr);                  // set CTRL Register Address
  ...
  {
    rtc_ram[i] = readIIC();        // ... and store it in rtc_ram
  }
}
...
void DS1307new::setRAM(uint8_t rtc_addr, uint8_t * rtc_ram, uint8_t rtc_quantity)
{
  ...
  rtc_addr &= 63;                       // avoid wrong adressing. Adress 0x08 is now address 0x00...
  rtc_addr += 8;                        // ... and address 0x3f is now 0x38
  printIIC(rtc_addr);                  // set RAM start Address 
  for(int i=0; i<rtc_quantity; i++)     // Send x data from given address upwards...
  {
    printIIC(rtc_ram[i]);              // ... and send it from rtc_ram to the RTC Chip
  }
  Wire.endTransmission();
}
